 ...
 * @prop {string} adjustWidth Optional expression used to adjust width values prior to sending to the layout service, evaluated using [expr-eval]{@link https://www.npmjs.com/package/expr-eval} with input variables 'w' and 'h'
 * @prop {string} adjustHeight Optional expression used to adjust height values prior to sending to the layout service, evaluated using [expr-eval]{@link https://www.npmjs.com/package/expr-eval} with input variables 'w' and 'h'
 * @prop {string} clientAdjustX Optional expression used to adjust x values (width and x offset) received from the layout service, evaluated using [expr-eval]{@link https://www.npmjs.com/package/expr-eval} with input variables 'x' and 'y'
 * @prop {string} clientAdjustY Optional expression used to adjust y values (height and y offset) received from the layout service, evaluated using [expr-eval]{@link https://www.npmjs.com/package/expr-eval} with input variables 'x' and 'y'
 */
...
DMAppLayoutRegionCtl.prototype.addLayoutRegions = function() {
	...
	for (let i = 0; i < arguments.length; i++) {
		...
		argCheck([], 0, this.logger, "addLayoutRegions: (" + i + ")", info, [
				...
				'suppressElementlessComponentWarning', 'columnMode', 'adjustWidth', 'adjustHeight',
