 ...
 * @param {boolean=} options.logToRingBuffer optional whether to store all log messages in a ring buffer, see {@link DMAppController#logRingBuffer}
 * @param {string=} options.sessionIdTemplate optional default session ID template, see: {@link DMAppController#generateRandomIdStringFromTemplate}
 */
function DMAppController(options) {
	...
	this.logToRingBuffer = false;
	this.sessionIdTemplate = null;
	...
	if (options) {
		...
		if (options.logToRingBuffer != null) this.logToRingBuffer = options.logToRingBuffer;
		if (options.sessionIdTemplate != null) this.sessionIdTemplate = options.sessionIdTemplate;
	}
	if (this.logWindowErrors === undefined) this.logWindowErrors = this.singleInstance;

	if (this.logToRingBuffer) {
		Object.defineProperties(this, {
			logRingBuffer:        { value: [] },
		});
	}
	...
	Object.defineProperties(this, {
		...
		_defaultSessionId:         { value: this.sessionIdTemplate ? this.generateRandomIdStringFromTemplate(this.sessionIdTemplate) : "/S" + this.generateRandomIdString(12) },
	...
	argCheck(arguments, 1, this.logger, "DMAppController constructor", options,
			['setDeviceId', 'deviceIdPrefix', 'deviceIdNamespace', 'deviceIdNamespaceGroup', 'defaultLogLevel', 'concatLogArgs', 'makeNewDeviceId', 'communalDevice', 'concurrentVideo',
					...
					'logToRingBuffer', 'sessionIdTemplate']);
...
/**
 * Returns the base websocket protocol for the current controller instance.
 * This is either "ws:" or "wss:"
 *
 * @returns {string} Protocol string
 */
DMAppController.prototype.getBaseWebsocketUrlProtocol = function() {
	if (location.protocol === "http:") {
		return "ws:";
	} else {
		return "wss:";
...
DMAppController.prototype._getUrl = function(prop) {
	...
	} else if (name.substr(0, 3) === 'w//') {
		name = this.getBaseWebsocketUrlProtocol() + name.slice(1);
	}
	return name;
};
...
/**
 * Generate a random ID string from a template
 * '?' characters in the template are replaced
 *
 * @param {string} template Template string, '?' characters are replaced
 * @returns {string} random ID string
 */
DMAppController.prototype.generateRandomIdStringFromTemplate = function(template) {
	return template.replace(/\?/g, this.generateRandomIdString.bind(this, 1));
};
...
DMAppController.prototype.createNamedLogger = function(name, noNetworkLogging) /* -> loglevel logger */ {
	...
	if (this.logToRingBuffer) {
		if (!params.messageOutputs) params.messageOutputs = [];
		params.messageOutputs.push(this._ringBufferLogMessage.bind(this));
	}
...
DMAppController.prototype._ringBufferLogMessage = function(args, methodName, methodLevel, logName) {
	if (this.logRingBuffer.length >= 4000) this.logRingBuffer.shift();
	this.logRingBuffer.push({
		logName: logName,
		methodName: methodName,
		methodLevel: methodLevel,
		logArguments: args,
	});
	this.emit("logRingBufferUpdate");
};

/**
 * Log message ring buffer entry
 *
 * @typedef {Object} DMAppController~LogRingBufferEntry
 * @prop {!Array} logArguments Arguments to log method
 * @prop {!string} methodName Log method name
 * @prop {!number} methodLevel Log method level
 * @prop {!string} loggerName Logger name
 */
/**
 * Log message ring buffer. This is populated ONLY if the `logToRingBuffer` constructor option is set.
 * Log message entries are appended to the end of the array (using `push()`).
 * The reader SHOULD read and consume messages from the front of the array (using `shift()`).
 * The event {@link DMAppController#event:logRingBufferUpdate} is fired when element(s) are appended.
 *
 * @member {Array.<DMAppController~LogRingBufferEntry>} DMAppController#logRingBuffer
 */
/**
 * Log message ring buffer update event, see: {@link DMAppController#logRingBuffer}
 *
 * @event DMAppController#logRingBufferUpdate
 */
...
DMAppController.prototype.getSharedStateMappingService = function() {
	...
	if (!self._sharedStateMappingServiceRc) {
		...
		const mappingService = SharedStateClient.MappingService(serviceUrl, {
			...
			socketIo: MiscUtil.makeSocketIOClient,
		});
...
/**
 * Set signal value to be the evaluation of an expression string evaluated using [expr-eval](https://www.npmjs.com/package/expr-eval)
 *
 ...
 * @param {!string} expression Expression evaluated using [expr-eval](https://www.npmjs.com/package/expr-eval)
...
/**
 * Return result of the evaluation of an expression string evaluated using [expr-eval](https://www.npmjs.com/package/expr-eval)
 *
 * This includes the following extra constants and functions:
 *
 * Constants:
