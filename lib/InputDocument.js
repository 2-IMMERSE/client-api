 ...
 * @prop {!string} type Permitted values include: 'select', 'optional', 'optional/string'
 * @prop {Array.<InputDocument.InputDocumentVariationSelectOption>} options Selection options, required when type is 'select'
 * @prop {InputDocument.InputDocumentObject} content Object to merge with the input document, when optional variation is enabled, permitted when type is 'optional' or a subtype thereof
 * @prop {string=} inputDocumentUrl URL of another input document to merge with this input document, permitted when type is 'optional' or a subtype thereof (applied before content). This is relative to {@link InputDocument.InputDocumentHandler#inputUrl}.
 ...
 * @prop {string=} replaceToken Value of the token which is replaced with the variation value, this defaults to $$, permitted when type is 'optional/string'
 */
/**
		...
		if (doc.variations) {
			...
			for (let i = 0; i < doc.variations.length; i++) {
				...
				if (v.type === "select") {
					...
					argCheck([], 0, this.logger, "Input document: '" + prefix + "'", v,
							['name', 'description', 'type', 'content', 'inputDocumentUrl', 'uiAdvanced', 'replaceToken']);
					if (!v.content && !v.inputDocumentUrl) this.logger.throwError("Input document: '" + prefix + ".content' and '" + prefix + ".inputDocumentUrl' are both missing");
					contentCheck(v, prefix);
					entry.opts.set(true, {
						name: true,
						...
						content: v.content,
						inputDocumentUrl: v.inputDocumentUrl,
						replaceToken: v.replaceToken || '$$',
					});
					entry.opts.set(false, {
						name: false,
						description: "Option: " + v.name + " disabled",
					});
					entry.replaceMode = true;
				} else {
...
InputDocumentHandler.prototype._handleVariations = function(doc) {
	...
	for (let [k, v] of this.variationSelections) {
		...
		const vkey = entry.replaceMode ? !!v : v;
		const item = entry.opts.get(vkey);
		if (!item) this.logger.throwError("No such variation option: ", k, "-->", vkey);
		item.replaceWith = v;
	...
	const applyOverlay = function(item, obj) {
		if (obj) {
			if (item.replaceToken) {
				const from = item.replaceToken;
				const to = item.replaceWith;
				const filter = function(node) {
					if (node == null) {
						...
						for (const [key, value] of Object.entries(node)) {
							out[key.replace(from, to)] = filter(value);
						}
						return out;
					} else if (typeof node === "string") {
						return node.replace(from, to);
					} else {
						return node;
					}
				};
	...
	return Promise.all(entries).then(function(items) {
		for (let i = 0; i < items.length; i++) {
			const item = items[i];
			applyOverlay(item, item.subdoc);
			applyOverlay(item, item.content);
		}
	});
};
