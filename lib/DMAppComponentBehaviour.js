...
const DMAppComponentBehaviour = {
	...
	initDMAppComponentEx: function(aux, dMAppController, id, typeName, config) /* -> void */ {
		...
		self.setExpectedConfigParameterType('string', '__writeTimingSignal', '__notRunnableBeforeTime', '__elementClass', '__componentTimelineClockSource', '__softStopOnSignal');
	...
	_effectiveParamChange: function(event) {
		if (event.newValue && typeof event.newValue === "object") {
			...
			if (event.newValue.__softStopOnSignal) {
				if (event.newValue.__softStopOnSignal !== this.__prevSoftStopOnSignal) {
					if (this.__SoftStopOnSignalRelease) this.__SoftStopOnSignalRelease();
					if (!this.__SoftStopOnSignal) {
						this.__SoftStopOnSignal = new Signal.SettableSignal(false, { boolean: true });
						this.softStopped.registerBlockerSignal(this.__SoftStopOnSignal, this.listenerTracker);
					}
					try {
						const signalInfo = this.dMAppController.getSignalByName(event.newValue.__softStopOnSignal);
						this.__SoftStopOnSignalRelease = signalInfo.unref;
						this.__SoftStopOnSignal.setSignalCopy(signalInfo.signal);
					} catch(e) {
						this.logger.error("Exception thrown in a parameter __softStopOnSignal handler: ", e);
						this.__SoftStopOnSignal.setValue(false);
					}
				}
			} else if (this.__SoftStopOnSignal) {
				this.__SoftStopOnSignal.setValue(false);
				this.__prevSoftStopOnSignal = null;
				if (this.__SoftStopOnSignalRelease) {
					this.__SoftStopOnSignalRelease();
					delete this.__SoftStopOnSignalRelease;
				}
			}
